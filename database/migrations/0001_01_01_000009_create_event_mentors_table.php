<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration {
    public function up(): void
    {
        Schema::create('event_mentors', function (Blueprint $table) {
            $table->id();
            $table->foreignId('event_id')->constrained()->cascadeOnDelete();
            $table->foreignId('user_id')->constrained()->cascadeOnDelete();
            $table->enum('role', ['mentor', 'co-mentor', 'speaker', 'moderator'])->default('mentor');
            $table->string('goal_title')->nullable();
            $table->text('goal_detail')->nullable();
            $table->enum('goal_status', ['planned', 'in_progress', 'achieved'])->default('planned');
            $table->unsignedInteger('target_participants')->nullable();
            $table->unsignedInteger('achieved_participants')->nullable();
            $table->string('materials_url')->nullable();
            $table->text('notes')->nullable();
            $table->timestamps();

            $table->unique(['event_id', 'user_id', 'role']);
        });
    }

    public function down(): void
    {
        Schema::dropIfExists('event_mentors');
    }
};
