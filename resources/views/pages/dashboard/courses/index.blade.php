...
@section('content')
    ...
    <script>
        // Publish warning modal functions
        function showPublishWarningModal(courseId) {
            const modal = document.getElementById('publish-warning-modal-' + courseId);
            if (modal) {
        ...
        document.addEventListener('keydown', function (e) {
            if (e.key === 'Escape') {
                closeEvidenceModal();
                // Close all publish warning modals
                document.querySelectorAll('[id^="publish-warning-modal-"]').forEach(modal => {
                    modal.classList.add('hidden');
                });
            }
        });
