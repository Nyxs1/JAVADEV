<header id="navbar"
    ...
    <div class="max-w-7xl mx-auto h-16 px-4 sm:px-8 flex items-center justify-between">
        ...
        {{-- Logo kiri --}}
        <a href="{{ route('index') }}" class="flex items-center gap-2 hover:opacity-80 transition-opacity">
            <img src="{{ asset('assets/images/logos/Logo-Photoroom.png') }}" alt="JavaDev Logo" class="h-12 w-auto">
        </a>

        {{-- Menu tengah --}}
        <nav class="hidden md:flex items-center gap-10 text-sm font-medium text-slate-600">
            <a href="{{ route('index') }}#courses" class="nav-link hover:text-slate-900">Course</a>
            <a href="{{ route('events.index') }}" class="nav-link hover:text-slate-900">Event</a>
            <a href="{{ route('index') }}#portfolio" class="nav-link hover:text-slate-900">Portfolio</a>
            <a href="{{ route('index') }}#leaderboard" class="nav-link hover:text-slate-900">Leaderboard</a>
        </nav>
        ...
        @auth
            ...
            <div class="relative" id="avatar-dropdown">
                <button type="button"
                    ...
                    {{-- Avatar menggunakan component reusable --}}
                    <x-avatar :user="auth()->user()" size="md" shape="circle" id="avatar-circle"
                        class="ring-2 ring-transparent transition-all" />
                </button>

                {{-- Dropdown Menu with Connector --}}
                <div class="avatar-dropdown-menu" id="avatar-menu" role="menu">
                    <div class="avatar-dropdown-content">
                        <a href="{{ route('users.dashboard', auth()->user()->username) }}"
                            ...
                            role="menuitem">
                            <img src="{{ asset('assets/icons/home.svg') }}" alt="" class="w-4 h-4 opacity-70">
                            Dashboard
                        </a>
                        ...
                        <a href="{{ route('profile.show', auth()->user()->username) }}"
                            class="flex items-center gap-3 px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 transition-colors"
                            role="menuitem">
                            <img src="{{ asset('assets/icons/user.svg') }}" alt="" class="w-4 h-4 opacity-70">
                            Profile
                        </a>
                        ...
                        <a href="{{ route('profile.settings') }}"
                            class="flex items-center gap-3 px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 transition-colors"
                            role="menuitem">
                            <img src="{{ asset('assets/icons/settings.svg') }}" alt="" class="w-4 h-4 opacity-70">
                            Settings
                        </a>
                        ...
                        <form method="POST" action="{{ route('logout') }}" class="block">
                            ...
                            <button type="submit"
                                ...
                                role="menuitem">
                                <img src="{{ asset('assets/icons/logout.svg') }}" alt="" class="w-4 h-4 opacity-70">
                                Logout
                            </button>
                        </form>
                    </div>
