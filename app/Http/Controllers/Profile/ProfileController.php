...
class ProfileController extends Controller
{
    ...
    public function update(UpdateProfileRequest $request)
    {
        ...
        return $this->respondWithSuccess(
            ...
            [
                ...
                'avatar_focus' => $result['avatar_focus'],
            ]
        );
    }
    ...
    {
        $validated = $request->validate([
            'panX' => 'required|numeric',
            'panY' => 'required|numeric',
            'zoom' => 'required|numeric|min:0.01|max:10',
        ]);

        $user = Auth::user();
        $user->avatar_focus = [
            'zoom' => floatval($validated['zoom']),
            'panX' => floatval($validated['panX']),
            'panY' => floatval($validated['panY']),
        ];
        $user->save();
